...
unsigned char *readBlockFromDisk(unsigned int addr, Buffer *buf)
{
	...
	sprintf(filename, "%d.blk", addr);
	FILE *fp = fopen(filename, "rb"); // changed from "r" to "rb, so that 0x1A will not be treated as EOF
	...
	*blkPtr = BLOCK_UNAVAILABLE;
	...
	while ((ch = fgetc(fp)) != EOF && bytePtr < blkPtr + buf->blkSize) // line 108 changed for here
	{
		*bytePtr = ch;
		bytePtr++;
	}
...
int writeBlockToDisk(unsigned char *blkPtr, unsigned int addr, Buffer *buf)
{
	...
	sprintf(filename, "%d.blk", addr);
	FILE *fp = fopen(filename, "wb"); // changed from "w" to "wb"
